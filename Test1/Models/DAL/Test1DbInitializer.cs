...
namespace Test1.Models.DAL
{
    public class Test1DbInitializer : DropCreateDatabaseAlways<ApplicationDbContext>
    {
        protected override void Seed(ApplicationDbContext db)
        {
            ...
            db.Orders.Add(new Order { Done = false, OrderId = 1, DateStart = DateTime.Today, CustomerId = db.Users.ToList().First().Id, Completed = false });
            db.Orders.Add(new Order { Done = false, OrderId = 2, DateStart = DateTime.Today, CustomerId = db.Users.ToList().First().Id, Completed = false });
            db.Orders.Add(new Order { Done = false, OrderId = 3, DateStart = DateTime.Today, CustomerId = db.Users.ToList().First().Id, Completed = false });
